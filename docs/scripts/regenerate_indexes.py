...
# SEO/AEO lint thresholds. See docs/scripts/README.md or the
# --strict flag below. Matches values agreed in the 2026-04-17 blog review.
TITLE_MAX = 65
DESC_MAX = 170
DESC_MIN = 120
...
# ---------------- static blog index (2026-09-03) ----------------
# docs/index.html used to be a hand-frozen list of 69 cards whose newest date
# was 2026-03-30; the 28 posts published after that had NO static inbound link
# anywhere on the site (the page fetches posts-data.json and re-renders in the
# browser, which is not a link Googlebot can count on). Derived now, like
# posts-data.json and sitemap.xml: never hand-edit (#65). The client-side
# renderer still runs for the category filters and overwrites the grid with the
# same cards.
INDEX_PATH = DOCS / "index.html"
INDEX_START = "<!-- POSTS:START (generated by regenerate_indexes.py — do not edit) -->"
INDEX_END = "<!-- POSTS:END -->"
BADGE_CLASS = {
...
def render_card(e, href_prefix="posts/"):
    """One blog card, same markup the index's JS renderer emits."""
    ...
    return (
        ...
        f'<h2><a href="{href_prefix}{e["slug"]}.html">{escape(e["title"])}</a></h2>'
...
def regenerate_index_html(entries):
    if not INDEX_PATH.exists():
        return 0
    html = INDEX_PATH.read_text(encoding="utf-8")
    cards = "\n".join(render_card(e) for e in entries)
    block = f"{INDEX_START}\n{cards}\n{INDEX_END}"
    if INDEX_START in html and INDEX_END in html:
        a = html.index(INDEX_START)
        b = html.index(INDEX_END) + len(INDEX_END)
        new = html[:a] + block + html[b:]
    else:
        # First run: replace everything between the grid's opening tag and the
        # bare `</div>` line that closes it (the legacy hand-written cards).
        m = re.search(r'(<div class="card-grid" id="posts-grid">)\n(.*?)\n</div>', html, re.S)
        if not m:
            print("WARNING: docs/index.html has no <div class=\"card-grid\" id=\"posts-grid\"> — index not regenerated", file=sys.stderr)
            return 0
        new = html[:m.start(2)] + block + html[m.end(2):]
    if new != html:
        INDEX_PATH.write_text(new, encoding="utf-8")
    # Self-check: the static card count must equal the entry count, or the
    # index is lying about the corpus again.
    a, b = new.index(INDEX_START), new.index(INDEX_END)
    n = new[a:b].count('<div class="card">')
    if n != len(entries):
        print(f"ERROR: docs/index.html renders {n} cards for {len(entries)} entries", file=sys.stderr)
        sys.exit(2)
    return n
...
def main():
    ...
    index_cards = regenerate_index_html(entries)
    ...
    for post_file in sorted(POSTS_DIR.glob("*.html")):
        ...
        if issues:
            ...
            for sev, msg in issues:
                # Collapse variable bits (char counts) so the per-check
                # summary groups by issue type, not by exact length.
